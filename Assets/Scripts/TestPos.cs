...
public class TestPos : MonoBehaviour
{
    ...
    async void SendMess()
    {
        await TransportHandler.Transport.SendTo(new DataPacket((byte)OperationCode.MyTransform, new Dictionary<ParameterCode, object> { { ParameterCode.X, transform.position.x }, { ParameterCode.Y, transform.position.y }, { ParameterCode.Z, 0 }, { ParameterCode.Id, TransportHandler.Transport.Id } }, SendClientFlag.All));
    }
